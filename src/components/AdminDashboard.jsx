...
function AdminDashboard() {
  ...
  const addUser = () => {
    const id = Date.now();
    setUsers([
      { id, name: `New User ${id % 1000}`, email: `user${id % 1000}@x.com`, role: "client" },
      ...users,
    ]);
  };
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-t hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span className={`px-2 py-1 text-xs rounded ${u.role==="therapist"?"bg-green-100 text-green-800":"bg-blue-100 text-blue-800"}`}>
                    {u.role}
                  </span>
                </td>
                <td className="p-3">
